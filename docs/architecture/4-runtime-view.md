...
### MCP KIT Interaction Diagrams

The MCP KIT add-on introduces new runtime flows for AI clients interacting with the dataspace via natural language. The following diagrams describe the key interaction patterns. The flows are represented as sequence diagrams below.
...
sequenceDiagram
    ...
    box IC-Hub
        participant M as MCP Kit (/addons/mcp-kit/mcp)
