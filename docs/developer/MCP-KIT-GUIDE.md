...
The **MCP KIT** is an IC-Hub backend add-on that exposes the hub's dataspace
...
[ADR 0005 — MCP KIT Add-On](../architecture/decision-records/0005-mcp-kit-addon.md).
For the runtime sequence diagrams, see the
[Runtime View](../architecture/4-runtime-view.md#mcp-kit-interaction-diagrams).

> **Status:** the MCP KIT is enabled by default. When disabled it adds no
...
|---------|--------------------|------|
| MCP tool surface | Streamable HTTP (MCP) | `{hostname}/addons/mcp-kit/mcp` |
| Liveness check | `GET` | `{hostname}/v1/addons/mcp-kit/health` |
| Audit log (admin) | `GET` | `{hostname}/v1/addons/mcp-kit/audit?limit=50` |
...
`addons.mcp_kit.mount_path`. The `health` and `audit` endpoints are regular
...
`{hostname}{mount_path}` and is used to build the OAuth 2.1
protected-resource metadata that clients auto-discover.

## Configuration

All keys live under `addons.mcp_kit` in `ichub-backend/config/configuration.yml`
...
addons:
  mcp_kit:
    ...
    enabled: true
    # Enable OAuth 2.1 (Keycloak) authentication for the MCP endpoint.
    ...
    redis_url: null
    audit:
      # Expose GET /v1/addons/mcp-kit/audit for admin-role users.
...
| `authorization.enabled` | `addons.mcp_kit.oauth_enabled` + `keycloak.enabled` | MCP auth behaviour |
|---|---|---|
| `false` | any | **No auth** — endpoint publicly accessible (dev/test only; logs a warning) |
| `true` | both `true` | **`KeycloakAuthProvider`** — OAuth 2.1 proxy with Dynamic Client Registration, JWT validation, plus API-key Bearer fallback |
...
1. **`JWTVerifier`** — validates Keycloak-issued JWTs against the realm JWKS
   endpoint (`{auth_url}/realms/{realm}/protocol/openid-connect/certs`).
...
### Keycloak setup (OAuth 2.1 / DCR)

The OAuth path uses **Dynamic Client Registration (DCR)** so MCP clients
register themselves on first connect — operators do **not** pre-create a
Keycloak client.

1. **Keycloak ≥ 26.6.0** — earlier versions have a DCR incompatibility with MCP
   clients (fixed in [keycloak#45309](https://github.com/keycloak/keycloak/pull/45309)).
2. **Enable anonymous client registration** on the realm:
   *Realm Settings → Client Registration → Anonymous access → Enabled*.
3. **Set `hostname`** to the publicly reachable IC-Hub URL — the MCP endpoint
   URL and OAuth metadata are derived from it.
4. **Enable the OAuth flags:**
   ```yaml
   authorization:
     enabled: true
     keycloak:
...
```json
{
  "mcpServers": {
    "ichub": {
      "url": "https://ichub.example.com/addons/mcp-kit/mcp"
    }
  }
}
```
...
- With **OAuth enabled**, Claude Desktop opens a browser to authenticate against
  Keycloak and registers itself via DCR — no manual token handling.
...
**"IC-Hub MCP KIT"** with usage instructions the client surfaces to the model.

### Microsoft Copilot Studio / custom agents

Point any MCP-capable client at the same streamable-HTTP URL. OAuth 2.1
discovery is served from `{hostname}/addons/mcp-kit/mcp/.well-known/oauth-protected-resource`
when OAuth is enabled.

### Local development (no auth)

For local testing, leave `authorization.enabled: false`. The endpoint is then
open at `http://localhost:9000/addons/mcp-kit/mcp` with no credentials. **Never
...
> exception); see [ADR 0005](../architecture/decision-records/0005-mcp-kit-addon.md).
...
`managers/addons_service/mcp_kit/v1/confirmation.py`.
...
`audit.expose_admin_endpoint` is `true`, recent records are available to
admin-role users:

```
GET /v1/addons/mcp-kit/audit?limit=50
```
...
| `404` on the MCP path | `addons.mcp_kit.enabled: false`, or a non-default `mount_path` the client isn't using. |
