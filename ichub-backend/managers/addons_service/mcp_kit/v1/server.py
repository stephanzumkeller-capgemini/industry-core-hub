...
# main FastAPI app at the path configured in addons.mcp_kit.mount_path.
...
from models.services.addons.mcp_kit.v1.tool_io import OdrlPolicy, QuerySpec

# Derive the public MCP endpoint URL for OAuth 2.1 protected-resource metadata.
...
_mount_path = str(ConfigManager.get_config("addons.mcp_kit.mount_path", "/addons/mcp-kit/mcp"))
_mcp_server_url = f"{_hostname}{_mount_path}"

_dtr_policies = get_dtr_policies()
_READ_ONLY = ToolAnnotations(readOnlyHint=True)

_auth_provider = create_mcp_auth_provider(_mcp_server_url)
...
async def list_known_partners(ctx: Context) -> list[dict]:
    ...
    """
    async with audit_logger.record_call(ctx.session_id, "list_known_partners", {}, _user_sub(ctx)) as meta:
...
async def list_partner_twins(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "list_partner_twins", {"bpnl": bpnl}, _user_sub(ctx)
    ) as meta:
        resolved_query = (
...
async def get_twin_details(ctx: Context, bpnl: str, twin_id: str) -> dict:
    ...
    """
    async with audit_logger.record_call(
        ctx.session_id, "get_twin_details", {"bpnl": bpnl, "twin_id": twin_id}, _user_sub(ctx)
    ) as meta:
...
async def list_twin_submodels(ctx: Context, bpnl: str, twin_id: str) -> list[dict]:
    ...
    """
    async with audit_logger.record_call(
        ctx.session_id, "list_twin_submodels", {"bpnl": bpnl, "twin_id": twin_id}, _user_sub(ctx)
    ) as meta:
...
async def list_my_catalog_parts(
    ...
    """
    async with audit_logger.record_call(
        ctx.session_id, "list_my_catalog_parts", {"manufacturer_id": manufacturer_id}, _user_sub(ctx)
    ) as meta:
...
async def fetch_dpp(
    ...
    """
    async with audit_logger.record_call(
        ctx.session_id, "fetch_dpp", {"dpp_id": dpp_id}, _user_sub(ctx)
    ) as meta:
...
async def create_catalog_part(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "create_catalog_part", tool_args, _user_sub(ctx)
    ) as meta:
...
async def update_catalog_part(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "update_catalog_part", tool_args, _user_sub(ctx)
    ) as meta:
...
async def create_serialized_part(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "create_serialized_part", tool_args, _user_sub(ctx)
    ) as meta:
...
async def share_catalog_part(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "share_catalog_part", tool_args, _user_sub(ctx)
    ) as meta:
...
async def register_business_partner(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "register_business_partner", tool_args, _user_sub(ctx)
    ) as meta:
...
async def create_catalog_part_twin(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "create_catalog_part_twin", tool_args, _user_sub(ctx)
    ) as meta:
...
async def create_serialized_part_twin(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "create_serialized_part_twin", tool_args, _user_sub(ctx)
    ) as meta:
...
async def attach_twin_aspect(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "attach_twin_aspect", tool_args, _user_sub(ctx)
    ) as meta:
...
async def share_dpp(
    ...
    async with audit_logger.record_call(
        ctx.session_id, "share_dpp", tool_args, _user_sub(ctx)
    ) as meta:
...
# Starlette ASGI sub-application for the streamable-HTTP MCP transport.
# Mounted into the FastAPI app in controllers/fastapi/app.py.
