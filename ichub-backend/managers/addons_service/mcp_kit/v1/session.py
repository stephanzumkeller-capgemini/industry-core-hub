...
# TTL is read from addons.mcp_kit.session_ttl_seconds (default 7200 s).
#
# For multi-replica deployments, replace with a RedisSessionStore by setting
# addons.mcp_kit.redis_url — hook is reserved here for Step 2 extension.

import time
from abc import ABC, abstractmethod

from managers.config.config_manager import ConfigManager
from models.services.addons.mcp_kit.v1.session import PendingConfirmation, SessionState
...
def create_session_store() -> SessionStore:
    """Factory — returns the correct store based on configuration."""
    ttl = ConfigManager.get_config("addons.mcp_kit.session_ttl_seconds", 7200)
    redis_url = ConfigManager.get_config("addons.mcp_kit.redis_url")
    if redis_url:
        raise NotImplementedError(
            "Redis session store is not yet implemented. "
            "Set addons.mcp_kit.redis_url to null to use the in-process store."
        )
