...
# Structured audit logger for MCP KIT tool calls.
#
...
# GET /v1/addons/mcp-kit/audit (see controllers/…/mcp_kit.py).
...
from models.services.addons.mcp_kit.v1.audit import AuditRecord

logger = LoggingManager.get_logger(__name__)

# Keys whose values are redacted before logging (never log credentials).
_REDACT_KEYS = frozenset({"governance", "api_key", "token", "password", "secret"})
...
audit_logger = AuditLogger(
    buffer_size=int(ConfigManager.get_config("addons.mcp_kit.audit.buffer_size", 500))
)
