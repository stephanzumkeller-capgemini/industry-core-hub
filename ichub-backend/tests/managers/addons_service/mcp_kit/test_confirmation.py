...
class TestSessionStorePendingConfirmation:
    ...
    def test_pending_confirmations_capped(self):
        ...
        """
        with patch(
            "managers.addons_service.mcp_kit.v1.session.ConfigManager.get_config",
            side_effect=lambda key, default=None: 2 if key == "addons.mcp_kit.max_pending_confirmations" else default,
        ):
            for i in range(3):
